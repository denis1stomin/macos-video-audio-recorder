...
- **Large output file size.** A ~1h10m full-screen recording at the original fixed encoding settings (native display resolution, VideoToolbox's own automatic bitrate) came out to ~2.4 GB — good quality, but too big for easy sharing/storage over long sessions.
  - 1st mitigation (resolution capped at 1080px, frame rate capped at 30fps, an explicit ~0.06 bits/pixel/frame video bitrate clamped 1.5–8 Mbps, audio switched to 64 kbps mono AAC): **cut file size but made video visibly blurry.** A real test recording at 1080x592 targeted 1.5 Mbps but VideoToolbox's rate control actually only spent ~518 kbps on that mostly-static screen content — not enough for legible on-screen text.
  - 2nd attempt (raised the target to ~0.15 bpp, clamped 3–10 Mbps, specifically to leave headroom for the undershoot): **still visibly blurry, and barely moved the real number** — a retest at the same 1080x592 targeted 3 Mbps and still only measured ~561 kbps actual. Confirmed `AVVideoAverageBitRateKey` is a soft target VideoToolbox's rate control can largely ignore on low-motion content, regardless of how high the requested number is — raising it further wasn't a reliable lever.
  - 3rd attempt (dropped the explicit bitrate setting entirely, letting VideoToolbox fall back to its own complexity-adaptive default): **still blurry, and it turned out bitrate was never the real problem.** The actual bug: the resolution cap caps the *longer edge* at 1080, which for a 16:9 screen gives ~1080x608 — not 1920x1080 ("1080p"), only about a third of real 1080p's pixel count. No bitrate setting could have fixed a frame that was genuinely too small to begin with. Confirmed by asking whether the blur looked like a low-res/soft image vs. compression blockiness — it was the former, which pointed straight at resolution, not bitrate.
  - **Fixed (4th attempt), not yet re-verified:** raised the resolution cap back to 1920, which now gives real 1920x1080 for a standard 16:9 screen. File size is still kept in check by the frame rate cap (30fps) and VideoToolbox's own adaptive bitrate (no explicit target, per the 3rd attempt above — that part stays). Audio (64 kbps mono AAC) is unaffected. Still no user-facing quality picker (e.g. High / Medium / "Optimized").
- **Black bars around shrunk video content after the captured window resizes (window-recording mode only).** Reported and reproduced: recording a specific app window, then something inside it goes fullscreen (e.g. a video player), or the user manually resizes the window — the encoded video keeps the dimensions locked in at recording start, so the new, differently-sized content only fills part of the frame, with the rest black.
  - 1st fix attempt (read each frame's real content size from ScreenCaptureKit's per-frame `contentRect`/`contentScale` info): **crashed the app** — a single noisy reading right after a segment started was trusted immediately, cascading through 3 rollovers down to a `0x0` "resize" that crashed `AVAssetWriter`.
  - 2nd fix attempt (same signal, but require 10 consecutive frames to agree before acting): didn't crash, but **the reported sizes were simply wrong** — a window resized to roughly half of a large main display came out as 232×150. Turned out `contentRect` describes where within the *already-fixed, stale* encoder canvas the content currently sits, not the window's true size — not a noisy signal, a wrong one (in the fullscreen case it even read backwards: growing content read as shrinking).
  - **3rd fix attempt, current, not yet re-verified:** stopped trying to infer size from frame metadata entirely. Now polls `SCShareableContent` every 1.5s for the recorded window's actual current frame (ground truth from the OS, not derived from our own stale configuration), requiring 2 consecutive polls to agree before acting, still with the 64px sanity floor.
