...
- **Large output file size.** A ~1h10m full-screen recording at the original fixed encoding settings (native display resolution, VideoToolbox's own automatic bitrate) came out to ~2.4 GB — good quality, but too big for easy sharing/storage over long sessions.
  ...
  - **3rd fix (current), not yet re-verified:** dropped the explicit bitrate setting entirely, letting VideoToolbox fall back to its own complexity-adaptive default — the same rate-control behavior that, before any of these encoding changes, was the one explicitly praised as good quality (the complaint was only ever file size, not quality, at that setting). File size is now controlled solely by the resolution cap (1080px) and frame rate cap (30fps), which reduce the pixel/frame volume fed to the encoder in the first place — a guaranteed reduction, unlike fighting the rate controller's soft bitrate target. Audio (64 kbps mono AAC) is unaffected. Still no user-facing quality picker (e.g. High / Medium / "Optimized").
