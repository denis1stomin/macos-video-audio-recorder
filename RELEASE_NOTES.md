...
- **Large output file size.** A ~1h10m full-screen recording at the original fixed encoding settings (native display resolution, VideoToolbox's own automatic bitrate) came out to ~2.4 GB — good quality, but too big for easy sharing/storage over long sessions. First mitigation (resolution capped at 1080px, frame rate capped at 30fps, an explicit ~0.06 bits/pixel/frame video bitrate clamped 1.5–8 Mbps, audio switched to 64 kbps mono AAC) **cut file size but made video visibly blurry** — a real test recording at 1080x592 targeted 1.5 Mbps but VideoToolbox's rate control actually only spent ~518 kbps on that mostly-static screen content (its "average bitrate" is a soft target it can undershoot considerably, not a floor it fills), and 518 kbps isn't enough for legible on-screen text. **Fixed (2nd attempt), not yet re-verified:** raised the video bitrate target well above what the pixel-count math alone suggests is needed, specifically to leave headroom for that undershoot — now ~0.15 bits/pixel/frame, clamped 3–10 Mbps. Resolution/frame-rate caps and the audio change are unaffected. Still no user-facing quality picker (e.g. High / Medium / "Optimized").
