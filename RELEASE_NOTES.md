...
- **Black bars around shrunk video content after the captured window resizes (window-recording mode only).** Reported and reproduced: recording a specific app window, then something inside it goes fullscreen (e.g. a video player) — the encoded video keeps the dimensions locked in at recording start, so the new, differently-sized content only fills part of the frame, with the rest black. First fix attempt (checking each frame's real content size via ScreenCaptureKit's per-frame `contentRect`/`contentScale` info, and rolling over to a new correctly-sized segment on a mismatch) **caused an app crash** when tested: a single noisy frame reading right after a segment started was trusted immediately, cascading through 3 rollovers in under 6 seconds down to a `0x0` "resize" that crashed `AVAssetWriter`. **Fixed (2nd attempt), not yet re-verified:** the resize check now requires the same new size to repeat for 10 consecutive frames before acting (filtering out single-frame noise) and rejects any reading below a 64px floor outright, never treating it as real. Whole-screen recording mode isn't affected, since a display's pixel size doesn't change mid-recording.
