...
- Testing strategy: two test targets. **`RecRexTests`** (scheme `RecRex`, run via `make test`) covers pure logic that doesn't need real screen/mic capture — file naming, segment rollover timing, the start-button label state machine — and is what **CI (GitHub Actions)** runs on every push/PR. **`RecRexCaptureTests`** (its own scheme, run via `make test-capture`) actually drives `RecordingManager` against the real ScreenCaptureKit/AVFoundation stack — starts a real few-second recording, stops it, and asserts the output is a non-empty, playable MP4 with the expected tracks. It needs real Screen Recording/Microphone permission granted locally to this app's signing identity, so it's local-only and intentionally excluded from both the `RecRex` scheme and CI. Any future manual QA checklist is still open.
